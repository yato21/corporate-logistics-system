...
const TaskCard = ({ task: initialTask, onEditOrder, userRole, onClose }) => {
  ...
  const handleDownloadDocument = async (doc) => {
    ...
    try {
      ...
      const serverUrl = 'http://localhost:5000'; // или process.env.REACT_APP_API_URL
      ...
      const response = await fetch(`${serverUrl}/uploads/${doc.filePath}`, {
        headers: {
      ...
      if (blob.type.includes('text/html')) {
        ...
        console.error('Получен HTML вместо файла:', text);
        throw new Error('Получен некорректный тип файла');
      }
    ...
    <>
      <div className="task-card" data-status={task.status}>
        ...
        <div className="task-card-body">
          ...
          {(task.comment || task.documents?.length > 0) && (
                          ...
                          {task.documents.map((doc) => (
                            ...
                            >
                              ...
                              <div className="flex-1">
                                <span className="text-truncate">{doc.originalName}</span>
                              </div>
                              <div>
